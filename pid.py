...
class PIDController:
    def __init__(self, kp, ki, kd, kt):
        ...
        self.ki = ki
        self.kd = kd
        self.kt = kt # temperature coefficient to account for more power at high temperature difference from room
    ...
    def update(self, target, value, dt):
        if dt == 0: return 0.0 # no change - avoid divide by 0 error
        error = target - value # calculate error
        self.integral += error * dt # add error to integral sum
        self.integral = max(-50, min(self.integral, 50)) # clamp integral sum to prevent too much accumulation
        derivative = (error - self.last_error) / dt # calculate derivative
        temp_diff = 20 - value # calculate temperature difference for constant heat dispersion at lower temperatures
        output = -self.kp * error + -self.ki * self.integral + -self.kd * derivative + self.kt * temp_diff
        output = max(self.min_output, min(output, self.max_output)) # clamp output to allowable values
...
target = 15
control = PIDController(0.5, 0.01, 0.15, 0.02)
last_time = time.time()

p27 = machine.Pin(27)
pwm27 = machine.PWM(p27)
pwm27.freq(500)
...
while True:
    ...
    if temp <= -1000:
        ...
        last_time = time.time()
        dt = 0.75
        out = control.update(target, temp, 0.75)
        pwm27.duty(int(out * 1023.0))
